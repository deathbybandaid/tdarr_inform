...
class Logs():
    ...
    def handler(self, *args):
        ...
        if method == "text":
            ...
            level = request.args.get('level', default=self.tdarr_inform.logger.levelname, type=str)
            limit = request.args.get('limit', default=None, type=str)

            logs = self.tdarr_inform.logger.memory.filter(level=level, limit=limit)

            fakefile = StringIO()

            for log_entry in list(logs.keys()):
                fakefile.write("%s\n" % self.tdarr_inform.logger.memory.dict[log_entry]["fmsg"])
