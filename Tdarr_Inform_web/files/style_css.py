...
class Style_CSS():
    ...
    def __init__(self, tdarr_inform):
        ...
        self.internal_style_file = pathlib.Path(
            self.tdarr_inform.config.internal["paths"]["www_dir"]).joinpath('style.css')
    ...
    def pull_external_theme(self):
        ...
        if self.tdarr_inform.config.dict["web_ui"]["theme"]:
            if self.tdarr_inform.config.dict["web_ui"]["theme"].startswith(tuple(["http://", "https://"])):
                css_req = self.tdarr_inform.web.session.get(self.tdarr_inform.config.dict["web_ui"]["theme"])
                self.external_style = StringIO(css_req.text)
                self.external_style_address = self.tdarr_inform.config.dict["web_ui"]["theme"]
    ...
    def handler(self, *args):
        ...
        if self.tdarr_inform.config.dict["web_ui"]["theme"]:
            if self.tdarr_inform.config.dict["web_ui"]["theme"] != self.external_style_address:
                self.pull_external_theme()
